...
public class RegistryService implements PathChildrenCacheListener {
    ...
    public void register(RegisterMetadata metadata){
        try {
            ...
            zookeeperClient.createEPhemeral(metadata.getPath() +
                    String.format(ZOOKEEPER_PROVIDER_CONSUMER_NODE, metadata.getAddress().getHost(), metadata.getAddress().getPort()), new ZookeeperClient.BackgroundCallback() {
                @Override
    ...
    public void unregister(RegisterMetadata metadata){
        try {
            zookeeperClient.delete(metadata.getPath() + String.format(ZOOKEEPER_PROVIDER_CONSUMER_NODE, metadata.getAddress().getHost(), metadata.getAddress().getPort()));
    ...
    private Address parse(String child){
        ...
        if(!StringUtils.isEmpty(child) && child.contains(":") && child.split(":").length == 2){
